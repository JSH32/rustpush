...
impl APNSConnection {
    ...
    pub async fn new(state: APNSStateInit) -> Result<APNSConnection, PushError> {
        let mut state = match state {
            APNSStateInit::New(data) => {
                let keypair = generate_push_cert(&data).await?;
                APNSState {
                    keypair,
                    token: None,
                }
            }
            APNSStateInit::State(state) => state,
        };
