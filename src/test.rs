...
async fn main() {
    ...
    let plist = Value::from_file("data.plist").expect("failed to read data.plist");

    let connection = Arc::new(
        APNSConnection::new(match &saved_state {
            Some(v) => APNSStateInit::State(v.push.clone()),
            None => APNSStateInit::New(plist),
        })
        .await
        .unwrap(),
    );
    ...
    if users[0].identity.is_none() {
        ...
        print!("Getting validation from API...");

        let validation = reqwest::get("https://validation-data.fly.dev/generate")
            .await
            .unwrap()
            .text()
            .await
