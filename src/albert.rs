...
fn build_activation_info(
    ...
    let serial_number = data
        .as_dictionary()
        .and_then(|dict| dict.get("iokit").unwrap().as_dictionary())
        .and_then(|iokit| iokit.get("IOPlatformSerialNumber").unwrap().as_string())
        .ok_or(PushError::MissingSerialError)
        .unwrap();
...
pub async fn generate_push_cert(data: &plist::Value) -> Result<KeyPair, PushError> {
    let private_key = PKey::from_rsa(Rsa::generate_with_e(
        1024,
        BigNum::from_u32(65537)?.as_ref(),
    )?)?;
    let activation_info = build_activation_info(private_key.as_ref(), data)?;
